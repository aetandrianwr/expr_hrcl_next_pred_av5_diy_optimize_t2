...
def get_dataset(paths_config, preprocess_config):
    ...
    Memory-efficient version with explicit cleanup of unused variables.
    """
    ...
    # Delete raw_df and geometry as they're no longer needed
    del raw_df, geometry
    gc.collect()
    ...
    if Path(quality_file).is_file():
        ...
        # Delete trips and tpls as they're no longer needed
        del trips, tpls
        gc.collect()

    # Delete pfs as it's no longer needed after quality filtering
    del pfs
    gc.collect()
    ...
    # Delete valid_user array as it's no longer needed
    del valid_user
    gc.collect()
    ...
    # Delete locs and filtered_locs after saving
    ...
    # Delete sp_merged after enrichment
    del sp_merged
    gc.collect()
...
def _filter_sp_history(sp, output_dir, seq_params):
    ...
    # Delete all_ids after extracting final_valid_id
    del all_ids
    gc.collect()
    ...
    # Delete intermediate user arrays
    del valid_users_train, valid_users_vali, valid_users_test
    gc.collect()

    filtered_sp = sp.loc[sp["user_id"].isin(valid_users)].copy()
    
    # Delete valid_users and original sp
    del valid_users, sp
    gc.collect()

    train_data, vali_data, test_data = split_dataset(filtered_sp)
    
    # Delete intermediate train/vali/test from first split (no longer needed)
    # Note: variables are reassigned above, so old values are automatically cleaned
    ...
    # Delete train_data, vali_data, test_data after encoding (only needed filtered_sp now)
